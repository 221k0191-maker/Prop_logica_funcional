% Hechos: herramienta(Nombre, [lista de características])
...
% Completar los 2 hechos que faltan (ejemplos)
...
herramienta(canva, [tipo(diseno), plataforma(web), plataforma(app), nivel(primaria), nivel(secundaria), nivel(universidad), licencia(freemium)]).
